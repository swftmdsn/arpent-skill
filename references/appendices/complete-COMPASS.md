...
> Historical complete router retained for rationale and edge-case recovery.
> `SKILL.md`, `references/workflows/COMPASS.md`, and the current mode documents
> are operationally authoritative; older mode-specific instructions below are
> not current behavior.

*Historical rationale for routing information in Arpent. Current operations use the compact skill, marker-selected mode, and current mode contracts.*
...
| Action requiring execution, tracking, or completion | Todo tool with `status: installed` | Structured DB plus readable Markdown trace |
...
| Optional cross-project session log | `06_indexes/memory/MEMORY.md`, only via explicit `--memory-log` | Disabled by default; each read requires a separate explicit request |
...
Both vault modes provide local continuity through project or area `_context.md`.
Full uses CLI-mediated `project create` and `session end`; minimal applies the
same structures through direct-file operation. Both retain all skills and
configuration, while minimal leaves coordinated modules dormant. Full-mode
delegated external memory still requires provider opt-in at the host level.
...
| `me.md` | Reading the user's authored interaction contract |
| **`COMPASS.md`** | Choosing the next operation and interaction path |
| `06_indexes/global_skills/arpent.skill.md` | Loading the compact vault-local operating method |
...
| `arpent <command> --help` | Exact CLI syntax when the compact vault-local skill does not cover an option |
| `06_indexes/tools.yaml` | Registered tools and declared status; not a command dispatcher |
| `06_indexes/global_skills/<tool>.skill.md` | Exact method for a tool with `status: installed` only |
...
3. **Load local context.** To resume in either mode, read in order: root
   ...
   optional log only after a separate explicit read request. Resume is this
   documentary protocol, not a command.
4. **Select execution from `.arpent`.** Full uses CLI-mediated operation.
   Minimal uses direct-file operation; `auto_full: true` may request the current
   guarded promotion behavior.
   CLI availability alone never overrides an explicit minimal choice.
...
2. **Load** only the relevant contract or the skill of a tool with `status: installed`.
3. **Resolve inputs** using the question policy in section 9.
4. **Read confirmation policy** from `06_indexes/cli/operations.yaml`.
   `always` requires confirmation before every mutation; `explicit-intent` runs explicit bounded
   requests directly and confirms high-impact or threshold-sized work; `never`
   skips additional confirmation while retaining technical checks.
...
expires_at: <dd-MM-YYYY-HH-mm or null>
...
| `created`, `modified` | `dd-MM-YYYY-HH-mm`, UTC |
| `description` | useful standalone summary or `null` |
| `type` | `note`, `concept`, `journal`, `log`, `checklist`, `reference`, `draft`, `template`, `meeting`, `idea`, `fleeting`, `linear`, `integration`, `angle`, `production`, `map`, `artefact` |
...
| `expires_at` | `dd-MM-YYYY-HH-mm` UTC or `null`; mostly for buffer items |
...
only after provider opt-in; interface availability alone does not establish it.
Arpent does not discover, select, configure, mirror, or synchronize memory
providers. When provider opt-in is absent, state that the item
...
In full, use `arpent project create <name> [--area <slug>] [--effort-cadence
heavylift|slowburn] [--effort-level low|medium|high]`. Minimal creates the same
structure directly. The human name becomes a visible lowercase ASCII kebab-case slug. The command creates a
...
- Dissolve only after at least one verified child. Pass `--yes` when the local
  confirmation policy requires confirmation; `never` does not pause for it.
...
Use `arpent usage report [--since <dd-MM-YYYY-HH-mm>] [--json]` for local
...
- Currently declared with `status: installed`: `todo` and the explicit
  `context_summary` workflow.
...
- `arpent sweep ephemeral [--dry-run]` is implemented and only processes
  ephemeral tools with `status: installed`. `delete-after-review` proposes; it
  never deletes.
...
- Cron has no daemon. Preview with `arpent cron run --tick --dry-run`; execution
  additionally requires `--allow-local-code`; the confirmation policy may also
  require `--yes`.
- Every enabled cron job must carry the unverified declaration
  `trust: local-code`. Treat its command as executable code with the user's OS
  permissions; enable jobs only in a vault you control.

### J. End a session

Use `arpent session end --summary <text> [--project <slug>] [--area <slug>]
[--decision <text> ...] [--next-step <text> ...] [--observation <text> ...]
[--trait <text> ...] [--memory-log]` in full. Minimal updates target context
directly. `--memory-log` records a one-use full-mode request to create or update
the optional cross-project `MEMORY.md`; reading that log later requires a
separate explicit request. A close
...
observations and traits in `pending_db_writes.yaml`; minimal does not operate
the queue. The full-mode queue is
...
supported in minimal mode because it needs coordinated state. Preserve the
...
a referenced destination is missing and creation was not explicitly requested; source
provenance is incoherent; or extraction, dissolution, manual archival, or a
control-plane change needs confirmation.
...
1. Apply the local confirmation policy and batch threshold; clarification remains separate.
